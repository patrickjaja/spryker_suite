...
$config[PropelConstants::ZED_DB_DATABASE] = $testEnvPrefix . getenv('POSTGRES_DATABASE');
...
foreach (explode(',', (string)getenv('APPLICATION_STORE_NAMES')) as $storename) {
    $config[RabbitMqEnv::RABBITMQ_CONNECTIONS][$storename] =
        [
            ...
            RabbitMqEnv::RABBITMQ_DEFAULT_CONNECTION => true, //ToDo: fix $CURRENT_STORE === $storename
        ];
}
...
$config[ApplicationConstants::BASE_URL_ZED] = sprintf(
    'https://%s%s',
    getenv('ZED_HOST') ?: 'not-configured-host',
...
$testEnvPrefix = APPLICATION_ENV === 'devtest' ? 'test-' : '';
...
$glueHost = getenv('GLUE_HOST') ?: 'localhost';
$gluePort = (int)(getenv('SPRYKER_API_PORT')) ?: 443;
$config[GlueApplicationConstants::GLUE_APPLICATION_DOMAIN] = getenv('GLUE_HOST');
